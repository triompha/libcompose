...
import (
	...
	composeYaml "github.com/triompha/libcompose/yaml"
	"gopkg.in/yaml.v2"
)
...
func Merge(existingServices *ServiceConfigs, environmentLookup EnvironmentLookup, resourceLookup ResourceLookup, file string, bytes []byte, options *ParseOptions) (string, string, map[string]*ServiceConfig, map[string]*VolumeConfig, map[string]*NetworkConfig, error) {
	if options == nil {
	...
	return config.Version, config.Scope, serviceConfigs, config.Volumes, config.Networks, nil
}
