...
import (
	...
	"github.com/triompha/libcompose/logger"
	"github.com/triompha/libcompose/lookup"
	"github.com/triompha/libcompose/project/events"
	"github.com/triompha/libcompose/utils"
	"github.com/triompha/libcompose/yaml"
)
...
func (p *Project) load(file string, bytes []byte) error {
	//	version, projectName, serviceConfigs, volumeConfigs, networkConfigs, err := config.Merge(p.ServiceConfigs, p.context.EnvironmentLookup, p.context.ResourceLookup, file, bytes, p.ParseOptions)
	version, _, serviceConfigs, volumeConfigs, networkConfigs, err := config.Merge(p.ServiceConfigs, p.context.EnvironmentLookup, p.context.ResourceLookup, file, bytes, p.ParseOptions)
	if err != nil {
